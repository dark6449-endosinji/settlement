...
const CampTab = ({
  ...
  const [costForm, setCostForm] = useState({ date: '', category: '행사비', description: '', amount: '' });
  ...
  // 회비 사용액 (비용 현황에서 '회비'로 입력된 항목이 차감)
  const membershipSpent = useMemo(
    () => campCosts.filter((c) => c.category === '회비').reduce((a, c) => a + c.amount, 0),
    [campCosts]
  );

  // 기부금 사용액 (비용 현황에서 '기부금'으로 입력된 항목이 차감)
  const donationSpent = useMemo(
    () => campCosts.filter((c) => c.category === '기부금').reduce((a, c) => a + c.amount, 0),
    [campCosts]
  );
  ...
  const totalSpent = eventSpent + membershipSpent + donationSpent;
  const totalRemain = totalBudget - totalSpent;
  const membershipRemain = membership - membershipSpent;
  const donationRemain = totalDonation - donationSpent;

  const categoryStyle = {
    행사비: 'bg-rose-100 text-rose-700',
    회비: 'bg-blue-100 text-blue-700',
    기부금: 'bg-emerald-100 text-emerald-700',
  };

  // 비용 현황 목록 (행사비/회비/기부금 전체)
  const mergedList = useMemo(() => {
    const fromSettlement = settlementEventCosts.map((i) => ({
      id: i.id, date: i.date, description: i.content || i.description || '',
      amount: i.amount, category: '행사비', _source: 'settlement',
    }));
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
          {[
            ...
            { label: '사용 현황', value: totalSpent, color: 'text-rose-600', iconBg: 'bg-rose-100 text-rose-500', icon: <TrendingUp size={16} />, prefix: '- ' },
        ...
        <div className="px-4 md:px-6 py-4 border-b border-gray-100 bg-gray-50/20">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            ...
            <div className="bg-white rounded-xl p-3 border border-gray-100">
              <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">사용액</p>
              <p className="font-bold text-rose-600 text-sm break-all">- ₩{fmt(membershipSpent)}</p>
            </div>
            <div className={`rounded-xl p-3 border ${membershipRemain >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
              <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">잔여</p>
              <p className={`font-bold text-sm break-all ${membershipRemain >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                ₩{fmt(Math.abs(membershipRemain))}
                {membershipRemain < 0 && <span className="text-[10px] ml-0.5">(초과)</span>}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-gray-100 bg-gray-50/10">
          ...
          <div className="px-4 md:px-6 py-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-sm">
              ...
              <div className="bg-white rounded-xl p-3 border border-gray-100">
                <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">사용액</p>
                <p className="font-bold text-rose-600 text-sm break-all">- ₩{fmt(donationSpent)}</p>
              </div>
              <div className={`rounded-xl p-3 border ${donationRemain >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
                <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">잔여</p>
                <p className={`font-bold text-sm break-all ${donationRemain >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                  ₩{fmt(Math.abs(donationRemain))}
                  {donationRemain < 0 && <span className="text-[10px] ml-0.5">(초과)</span>}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {/* 직접 입력 폼 (관리자) */}
        {isAdmin && (
          <form onSubmit={handleAddCost} className="px-4 md:px-6 py-4 grid grid-cols-2 md:grid-cols-5 gap-3 border-b border-gray-100 bg-gray-50/20">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-gray-500">날짜</label>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select name="category" value={costForm.category} onChange={handleCostChange}
        ...
        <div className="block md:hidden divide-y divide-gray-100">
              ...
              <div key={`${row._source}-${row.id}`}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${categoryStyle[row.category] || 'bg-gray-100 text-gray-600'}`}>{row.category}</span>
                    {isSettlement
          ...
          {mergedList.length === 0 && <div className="p-8 text-center text-gray-400 text-sm">비용 내역이 없습니다.</div>}
        </div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                return (
                  <tr key={`${row._source}-${row.id}`} className={`${isSettlement ? 'bg-indigo-50/20' : 'hover:bg-gray-50'} transition-colors`}>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${categoryStyle[row.category] || 'bg-gray-100 text-gray-600'}`}>
              ...
              {mergedList.length === 0 && (
                <tr><td colSpan={isAdmin ? 6 : 5} className="px-6 py-12 text-center text-gray-400">비용 내역이 없습니다.</td></tr>
              )}
            </tbody>
            {mergedList.length > 0 && (
